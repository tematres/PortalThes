...
function HTMLlistSources($CFG_MARC21, $mainVocab)
{
    ...
    if (configValue($CFG_MARC21["t750_z"])) {
        ...
        if (is_object($data)) {
            $rows.='	<li>X50'.$CFG_MARC21["subtag"].'z: <a target="_blank" href="'.$data->result->uri.'" title="'.$data->result->title.'">'.$data->result->title.'</a></li>';
        }
    }
    if (configValue($CFG_MARC21["t750_v"])) {
        ...
        if (is_object($data)) {
            $rows.='	<li>X50'.$CFG_MARC21["subtag"].'v: <a target="_blank" href="'.$data->result->uri.'" title="'.$data->result->title.'">'.$data->result->title.'</a></li>';
        }
    }
    if (configValue($CFG_MARC21["t750_x"])) {
        $data=getURLdata($CFG_MARC21["t750_x"].'?task=fetchVocabularyData');
        if (is_object($data)) {
            $rows.='	<li>X50'.$CFG_MARC21["subtag"].'x: <a target="_blank" href="'.$data->result->uri.'" title="'.$data->result->title.'">'.$data->result->title.'</a></li>';
        }
    }
