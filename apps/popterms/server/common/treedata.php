...
if($_GET["node"]){
	...
	foreach ($data->result->term as $value){
				...
				$link='<a href="'.$CFG_URL_PARAM["url_site"].$CFG_POP["PATH"].'index.php?task=fetchTerm&arg='.$value->term_id.'" title="'.$value->string.'">'.$value->string.'</a>';
	...
	foreach ($data->result->term as $value){
				...
				$link='<a href="'.$CFG_URL_PARAM["url_site"].$CFG_POP["PATH"].'index.php?task=fetchTerm&arg='.(int) $value->term_id.'" title="'.(string) $value->string.'">'.(string)$value->string.'</a>';
				array_push($array_data, array("label"=>"$link",
                  "id"=>"$value->term_id",
                  "load_on_demand"=>true));
			}
